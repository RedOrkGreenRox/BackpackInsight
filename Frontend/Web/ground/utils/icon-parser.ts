 ...
 * Поддерживает русские и английские термины и кириллические символы в регулярных выражениях.
 */
...
const START_LINE_KEYWORDS = ['This', 'Этот', 'Эта', 'Это'];
const ITEM_WORD_VARIANTS = 'items?|Items?|предмет(?:ы|ов|а)?';
...
const ALIAS_MAP: { [key: string]: string } = {
    ...
    // Russian keywords (5.1.0 Localization)
    'Оружие ближнего боя': 'MeleeWeapon', 'Оружие дальнего боя': 'RangedWeapon',
    'Расход выносливости': 'StaminaUsage', 'Восстановление выносливости': 'StaminaRecovery',
    'Шанс крита': 'CritChance', 'Крит. урон': 'CritDamage', 'Макс. здоровье': 'MaxHealth',
    'Аксессуар': 'TypeAccessory', 'Ингредиент': 'TypeIngredient', 'Минерал': 'TypeMineral',
    'Зелье': 'TypePotion', 'Еда': 'TypeFood', 'Рыба': 'TypeFish', 'Растение': 'TypePlant',
    'Череп': 'TypeSkull', 'Амулет': 'TypeCharm', 'Сумка': 'TypeBag', 'Инструмент': 'TypeTool',
    'Золото': 'Gold', 'Выносливость': 'Stamina', 'Точность': 'Accuracy', 'Перезарядка': 'Cooldown',
    'Яд': 'Poison', 'Ожог': 'Burn', 'Кровотечение': 'Bleed', 'Ослепление': 'Blind', 'Ускорение': 'Haste',
    'Заморозка': 'Chill', 'Проклятие': 'Curse', 'Регенерация': 'Regeneration', 'Краш-Тест Дакки': 'CrashTestDucky'
};
...
const HEADER_EXCEPTIONS_NO_COLON = ['are', 'is', 'can', 'require', 'also', 'rerolls', 'gain', 'inflict', 'steal', 'benefits', 'counts', 'per', 'для', 'при', 'каждые'];
...
const FORBIDDEN_NAMES = new Set([
    ...
    'Получить', 'Наложить', 'Снять', 'Каждые', 'При', 'Если', 'Когда', 'После', 'Использовать', 'Увеличить'
]);

const KEYWORDS_REGEX = /\b(Discharge|common|magnetic|Sabotage|empty|Lumps\s+of\s+Coal|Hunter['’]s\s+mark|Call\s+of\s+the\s+Void|пустой|обычный|магнитный)\b/gi;
const ITEM_PREFIX_KEYWORDS = ['magnetic', 'common', 'магнитный', 'обычный'];
const ITEM_SCALING_PREPOSITIONS = ['per', 'for each', 'за каждый', 'за каждое', 'за карту'];
...
const FORCED_HEADER_KEYWORDS = ['Quantity', 'Количество', '• After', '• После'];
const HEADER_BLOCK_KEYWORDS = ['If', 'Если'];
const NUMBER_FOLLOW_TRIGGERS = `(%|seconds?|сек\\.?|quantity|uses?|использования?|less|simple|common|magnetic|random|free|or|или|${ITEM_WORD_VARIANTS}|more|by|max|min|enemy|врага?|\\[\\[ICN|[A-Za-zА-Яа-яёЁ]|\\)|$)`;
...
export function parseTextWithIcons(text: string | undefined | null): string {
    ...
    let processedText = text.replace(/\\(([^)]+)\\)/g, (_, content: string) => content.replace(/[^a-zA-Zа-яА-ЯёЁ]/g, ''));
    const iconsMap = new Map<string, string>();
    let iconCounter = 0;

    // ШАГ 1 & 2: Скрытие иконок (поддержка кириллицы)
    let i = 0, tempText = '';
    while (i < processedText.length) {
        ...
        for (const word of ALL_TRIGGER_WORDS) {
            if (remaining.startsWith(word)) {
                ...
                if (!nextChar || !/[a-zA-Zа-яА-ЯёЁ]/.test(nextChar) || isConcatenated) {
    ...
    processedText = processedText.replace(/(^|\n|\\n)([A-ZА-Я][a-zа-яёЁ]+\s+)([\d.]+(?:-[\d.]+)?)/g, (m, s, w, n) => n.includes('span') ? m : `${s}${w}<span class="value-text">${n}</span>`);

    const itemNamesRegex = /(?<!\()\b([A-ZА-Я][a-zа-яёЁ]+(?:['’]s)?((?:\s+of)?\s+[A-ZА-Я][a-zа-яёЁ]+(?:['’]s)?){0,3})\b/g;
    ...
    iconsMap.forEach((html, placeholder) => {
        if (html.includes(`title="${SPECIAL_LOGIC_ICON}"`)) {
            const followWordRegex = new RegExp(`(${placeholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[ \xA0\t]*)(${ITEM_WORD_VARIANTS}|[A-ZА-Я][a-zа-яёЁ]+(?:\\s+[A-ZА-Я][a-zа-яёЁ]+)*)`, 'g');
    ...
    // ШАГ 6.2: Окрашивание слова item/items/предмет
    ...
    const finalIconCleanup = new RegExp(`(title="${SPECIAL_LOGIC_ICON}".*?<\\/picture>\\s*)(${ITEM_WORD_VARIANTS}|[A-ZА-Я][a-zа-яёЁ]+(?:\\s+[A-ZА-Я][a-zа-яёЁ]+)*)\\b`, 'g');
